...
export default function Home() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const storyResponse = await axios.post('https://gigagen.pythonanywhere.com/moral_to_modern_story', {
        k: 3,
        modern_values: text,
      });

      if (storyResponse.data && storyResponse.data.modern_story) {
        setFeed(storyResponse.data.modern_story);
        const newUtterance = new SpeechSynthesisUtterance(storyResponse.data.modern_story);
        setUtterance(newUtterance);

        const titleResponse = await axios.post('https://gigagen.pythonanywhere.com/story_title_generation', {
          story: storyResponse.data.modern_story,
        });
